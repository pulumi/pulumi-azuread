// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.azuread.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.Boolean;
import java.lang.String;
import java.util.Objects;


public final class GroupWithoutMembersDynamicMembershipArgs extends com.pulumi.resources.ResourceArgs {

    public static final GroupWithoutMembersDynamicMembershipArgs Empty = new GroupWithoutMembersDynamicMembershipArgs();

    /**
     * Whether rule processing is &#34;On&#34; (true) or &#34;Paused&#34; (false).
     * 
     */
    @Import(name="enabled", required=true)
    private Output<Boolean> enabled;

    /**
     * @return Whether rule processing is &#34;On&#34; (true) or &#34;Paused&#34; (false).
     * 
     */
    public Output<Boolean> enabled() {
        return this.enabled;
    }

    /**
     * The rule that determines membership of this group. For more information, see official documentation on [membership rules syntax](https://docs.microsoft.com/en-gb/azure/active-directory/enterprise-users/groups-dynamic-membership).
     * 
     * &gt; **Dynamic Group Memberships** Remember to include `DynamicMembership` in the set of `types` for the group when configuring a dynamic membership rule. Dynamic membership is a premium feature which requires an Azure Active Directory P1 or P2 license.
     * 
     */
    @Import(name="rule", required=true)
    private Output<String> rule;

    /**
     * @return The rule that determines membership of this group. For more information, see official documentation on [membership rules syntax](https://docs.microsoft.com/en-gb/azure/active-directory/enterprise-users/groups-dynamic-membership).
     * 
     * &gt; **Dynamic Group Memberships** Remember to include `DynamicMembership` in the set of `types` for the group when configuring a dynamic membership rule. Dynamic membership is a premium feature which requires an Azure Active Directory P1 or P2 license.
     * 
     */
    public Output<String> rule() {
        return this.rule;
    }

    private GroupWithoutMembersDynamicMembershipArgs() {}

    private GroupWithoutMembersDynamicMembershipArgs(GroupWithoutMembersDynamicMembershipArgs $) {
        this.enabled = $.enabled;
        this.rule = $.rule;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GroupWithoutMembersDynamicMembershipArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GroupWithoutMembersDynamicMembershipArgs $;

        public Builder() {
            $ = new GroupWithoutMembersDynamicMembershipArgs();
        }

        public Builder(GroupWithoutMembersDynamicMembershipArgs defaults) {
            $ = new GroupWithoutMembersDynamicMembershipArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param enabled Whether rule processing is &#34;On&#34; (true) or &#34;Paused&#34; (false).
         * 
         * @return builder
         * 
         */
        public Builder enabled(Output<Boolean> enabled) {
            $.enabled = enabled;
            return this;
        }

        /**
         * @param enabled Whether rule processing is &#34;On&#34; (true) or &#34;Paused&#34; (false).
         * 
         * @return builder
         * 
         */
        public Builder enabled(Boolean enabled) {
            return enabled(Output.of(enabled));
        }

        /**
         * @param rule The rule that determines membership of this group. For more information, see official documentation on [membership rules syntax](https://docs.microsoft.com/en-gb/azure/active-directory/enterprise-users/groups-dynamic-membership).
         * 
         * &gt; **Dynamic Group Memberships** Remember to include `DynamicMembership` in the set of `types` for the group when configuring a dynamic membership rule. Dynamic membership is a premium feature which requires an Azure Active Directory P1 or P2 license.
         * 
         * @return builder
         * 
         */
        public Builder rule(Output<String> rule) {
            $.rule = rule;
            return this;
        }

        /**
         * @param rule The rule that determines membership of this group. For more information, see official documentation on [membership rules syntax](https://docs.microsoft.com/en-gb/azure/active-directory/enterprise-users/groups-dynamic-membership).
         * 
         * &gt; **Dynamic Group Memberships** Remember to include `DynamicMembership` in the set of `types` for the group when configuring a dynamic membership rule. Dynamic membership is a premium feature which requires an Azure Active Directory P1 or P2 license.
         * 
         * @return builder
         * 
         */
        public Builder rule(String rule) {
            return rule(Output.of(rule));
        }

        public GroupWithoutMembersDynamicMembershipArgs build() {
            if ($.enabled == null) {
                throw new MissingRequiredPropertyException("GroupWithoutMembersDynamicMembershipArgs", "enabled");
            }
            if ($.rule == null) {
                throw new MissingRequiredPropertyException("GroupWithoutMembersDynamicMembershipArgs", "rule");
            }
            return $;
        }
    }

}
