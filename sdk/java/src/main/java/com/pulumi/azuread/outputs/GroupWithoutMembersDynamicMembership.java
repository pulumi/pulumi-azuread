// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.azuread.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.exceptions.MissingRequiredPropertyException;
import java.lang.Boolean;
import java.lang.String;
import java.util.Objects;

@CustomType
public final class GroupWithoutMembersDynamicMembership {
    /**
     * @return Whether rule processing is &#34;On&#34; (true) or &#34;Paused&#34; (false).
     * 
     */
    private Boolean enabled;
    /**
     * @return The rule that determines membership of this group. For more information, see official documentation on [membership rules syntax](https://docs.microsoft.com/en-gb/azure/active-directory/enterprise-users/groups-dynamic-membership).
     * 
     * &gt; **Dynamic Group Memberships** Remember to include `DynamicMembership` in the set of `types` for the group when configuring a dynamic membership rule. Dynamic membership is a premium feature which requires an Azure Active Directory P1 or P2 license.
     * 
     */
    private String rule;

    private GroupWithoutMembersDynamicMembership() {}
    /**
     * @return Whether rule processing is &#34;On&#34; (true) or &#34;Paused&#34; (false).
     * 
     */
    public Boolean enabled() {
        return this.enabled;
    }
    /**
     * @return The rule that determines membership of this group. For more information, see official documentation on [membership rules syntax](https://docs.microsoft.com/en-gb/azure/active-directory/enterprise-users/groups-dynamic-membership).
     * 
     * &gt; **Dynamic Group Memberships** Remember to include `DynamicMembership` in the set of `types` for the group when configuring a dynamic membership rule. Dynamic membership is a premium feature which requires an Azure Active Directory P1 or P2 license.
     * 
     */
    public String rule() {
        return this.rule;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(GroupWithoutMembersDynamicMembership defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private Boolean enabled;
        private String rule;
        public Builder() {}
        public Builder(GroupWithoutMembersDynamicMembership defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.enabled = defaults.enabled;
    	      this.rule = defaults.rule;
        }

        @CustomType.Setter
        public Builder enabled(Boolean enabled) {
            if (enabled == null) {
              throw new MissingRequiredPropertyException("GroupWithoutMembersDynamicMembership", "enabled");
            }
            this.enabled = enabled;
            return this;
        }
        @CustomType.Setter
        public Builder rule(String rule) {
            if (rule == null) {
              throw new MissingRequiredPropertyException("GroupWithoutMembersDynamicMembership", "rule");
            }
            this.rule = rule;
            return this;
        }
        public GroupWithoutMembersDynamicMembership build() {
            final var _resultValue = new GroupWithoutMembersDynamicMembership();
            _resultValue.enabled = enabled;
            _resultValue.rule = rule;
            return _resultValue;
        }
    }
}
